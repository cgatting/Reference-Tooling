...
class NLPEngine:
    def __init__(self):
        ...
        self.kw_extractor = yake.KeywordExtractor(
            lan="en", n=3, dedupLim=0.7, windowsSize=1
        )
    ...
    def extract_concepts(self, text: str) -> Dict[str, Any]:
        ...
        summary = self.summarizer(text, max_length=130, min_length=30, truncation=True)[0]['summary_text']
...
class CrossRefClient:
    def __init__(self, session: aiohttp.ClientSession):
        ...
        self.headers = {
            'User-Agent': 'CitationAssistant/1.0 (contact@example.com)'
        }
        
    async def search(self, query: str, rows: int = 3) -> List[Dict]:
        params = {
            'query': query,
            'rows': rows,
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                bib_database = bibtexparser.loads(
                    self.state.bib_file.read_text(encoding='utf-8')
                )
                
                # Extract sections and process
                ...
                for i, section in enumerate(sections, 1):
                    ...
                    for sentence in re.split(r'(?<=[.!?])\s+', section):
                        ...
                        for paper in papers:
                            ...
                            async with session.get(f"https://doi.org/{doi}") as response:
                                if response.status != 200:
                                    continue
                                ...
                                if similarity > threshold:
                                    self._update_citation(
                                        paper, sentence, tex_content, bib_database
                                    )
                
                # Save updated files
                self.state.tex_file.write_text(tex_content, encoding='utf-8')
                self.state.bib_file.write_text(
